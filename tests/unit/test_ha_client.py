...
class TestHomeAssistantClient:
    ...
    def test_get_all_entities(self, mock_ha_client):
        """Test retrieving all entities"""
        entities = mock_ha_client.get_all_entities()

        assert len(entities) > 0
        assert all("entity_id" in e for e in entities)
        assert all("state" in e for e in entities)

    def test_get_domotics_entities(self, mock_ha_client):
        """Test filtering for domotics entities"""
        entities = mock_ha_client.get_domotics_entities()
    ...
    def test_get_entity_state(self, mock_ha_client):
        """Test getting specific entity state"""
        state = mock_ha_client.get_entity_state("light.living_room")

        assert state is not None
        assert state["entity_id"] == "light.living_room"
        assert "state" in state

    def test_get_entity_state_not_found(self, mock_ha_client):
        """Test getting non-existent entity"""
        state = mock_ha_client.get_entity_state("light.nonexistent")
        assert state is None

    def test_call_service_turn_on(self, mock_ha_client):
        ...
        # Call service
        result = mock_ha_client.call_service(
            domain="light",
            service="turn_on",
    ...
    def test_call_service_with_data(self, mock_ha_client):
        ...
        state = mock_ha_client.get_entity_state("climate.living_room")
        assert state["attributes"]["temperature"] == 24

    def test_call_service_records_history(self, mock_ha_client):
        """Test that service calls are recorded"""
        mock_ha_client.reset_services_called()

        mock_ha_client.call_service("light", "turn_on", "light.living_room")
        mock_ha_client.call_service("climate", "turn_off", "climate.living_room")
    ...
    def test_connection(self, mock_ha_client):
        """Test connection check"""
        assert mock_ha_client.test_connection() is True
