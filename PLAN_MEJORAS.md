...
- [ ] Revisar el `index.lock` colgado en `.git/worktrees/kza-wt-escritorio/`: si no hay proceso git activo, eliminarlo y verificar `git -C ../kza-wt-escritorio status`.
...
- [ ] Crear `docs/SERVER_CONVENTIONS.md`: espejo local de las convenciones del servidor compartido que hoy viven solo en Notion (pág. 8 y 9.x — usuarios/UID, sub-rangos de puertos, Podman rootless + Quadlets, GPU por CDI). **[HUMANO]** pegar/validar el contenido desde Notion si Claude no tiene acceso al MCP.
