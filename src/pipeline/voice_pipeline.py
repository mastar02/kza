...
class VoicePipeline:
    ...
    # Frases para sugerencias de automatización
    SUGGESTION_ACCEPT = [
    ...
    async def process_command(self, audio: np.ndarray) -> dict:
        """
        Procesar un comando de audio completo.

        Si el orquestador está habilitado, usa el sistema multi-usuario.
        Si no, usa procesamiento legacy.
    ...
    async def _process_command_orchestrated(self, audio: np.ndarray) -> dict:
        """Procesar comando con orquestador multi-usuario."""
        result = {
            "text": "",
            "intent": None,
            "action": None,
        ...
        # 1. Procesar comando (STT + Speaker ID + Emotion en paralelo)
        ...
        permission_level = user.permission_level.value if user else 3
    ...
    async def _process_command_legacy(self, audio: np.ndarray) -> dict:
        """Procesamiento legacy (single-user) para compatibilidad."""
        result = {
            "text": "",
            "intent": None,
            "action": None,
        ...
        # 3. Verificar sugerencia pendiente
        ...
        # Primero intentar con el nuevo handler de voz (más completo)
        ...
        # Fallback al RoutineManager original (compatibilidad)
        ...
        # 8. Buscar en vector DB (con cache)
        ...
        if command:
            ...
            success = await self.ha.call_service_ws(
                ...
                command["entity_id"],
                command.get("data")
            ) if self._ws_connected else self.ha.call_service(
                command["domain"],
                command["service"],
                command["entity_id"],
                command.get("data")
            ...
            # Hablar respuesta con ajustes de emoción
            emotion_adj = emotion.response_adjustment if emotion else None
        ...
        if self.router:
            try:
                ...
                # Clasificar y responder en UNA sola inferencia (ahorra ~20ms)
        ...
        if not use_deep and router_response:
            # Router ya generó la respuesta, no necesita segunda inferencia
    ...
    def _check_feedback(self, text: str) -> dict:
        """Detectar si es feedback sobre respuesta anterior."""
    ...
    def _is_sync_command(self, text: str) -> bool:
        """Detectar comando de sincronización."""
        sync_phrases = [
    ...
    def _maybe_present_suggestion(self):
        """Presentar sugerencia de automatización."""
    ...
    def _get_conversation_context(self) -> str:
        """Obtener contexto breve para el router (optimizado para velocidad)."""
        context_parts = []
        ...
        # Usuario actual (si identificado)
        if hasattr(self, '_current_user') and self._current_user:
            context_parts.append(f"Usuario: {self._current_user}")
...
Usa el contexto de memoria para dar respuestas personalizadas. Sé breve y útil."""
...
Asistente:"""
    ...
    def _add_to_cache(self, key: str, value: dict):
        """Agregar a cache con límite de tamaño."""
