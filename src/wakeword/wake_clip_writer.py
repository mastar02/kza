...
class WakeClipWriter:
    ...
    def submit(self, room_id: str, score: float, audio) -> bool:
        ...
        try:
            ...
            self._queue.put_nowait((room_id, float(score), clip, time.time()))
            return True
        except queue.Full:
            return False
    ...
    def _write(self, room_id: str, score: float, clip: np.ndarray, ts: float) -> None:
        ...
        with wave.open(str(self._dir / name), "wb") as f:
            f.setnchannels(1)
            f.setsampwidth(2)
